  ...
  <b-container fluid class="pr-5">
    ...
    <b-sidebar id="sidebar-1" shadow z-index="50" no-header @shown="reduceGrid" @hidden="expandGrid">
        ...
        <b-card-header header-tag="header" role="tab">
          <b-button-group>
            <b-button block v-b-toggle.accordion-1 :disabled="add_widget_step == 0 ? false : true" variant="light">
              {{ this.add_widget_step > 0 ? this.new_widget.SignalID : 'Sensores' }}</b-button>
      ...
      <b-card no-body class="mb-1">
        <b-card-header header-tag="header" role="tab">
          <b-button-group>
            <b-button block v-b-toggle.accordion-3 :disabled="this.add_widget_step == 2 ? false : true" variant="light">
              Parámetros</b-button>
        ...
        <b-collapse id="accordion-3" accordion="my-accordion" role="tabpanel"
          ...
          <b-card-body>
            <b-form-group v-if="this.new_widget.type == 'PlotlyGrafica' || this.new_widget.type == 'EchartsGrafica'"
              id="timesteps" description="h = horas, m = minutos, s = segundos" label="Ventana de tiempo"
              label-for="timesteps">
              <!--                 
            ...
            <div v-else>No hay párametros disponibles</div>
...
export default {
  ...
  methods: {
    ...
    removeWidget(index) {
      ...
      const layoutIndex = this.layout.map(item => item.i).indexOf(index);
      this.layout.splice(layoutIndex, 1);

      const widgetIndex = this.widgets.map(widget => widget.gridindex).indexOf(index);
